...
parse_fp_fxn <- function (previous_election, state, elect_year) {
  ...
  } else if (state == "ACT" & elect_year < 1974) {
    clean_fp_df <- clean_fp_df %>% mutate(division_name = rep("AUSTRALIAN CAPITAL TERRITORY", nrow(clean_fp_df)))
  } else {clean_fp_df <- clean_fp_df}
...
parse_tcp_fxn <- function (state, elect_year) {
  ...
  # ACT before 1974 is an entity on its own
  if (state == "ACT" & elect_year < 1974) {
    largest <- max(clean_fp_df$fp_vote_share, na.rm = T)
    second_largest <- sort(clean_fp_df$fp_vote_share, 
                           partial = length(clean_fp_df$fp_vote_share)-1)[length(clean_fp_df$fp_vote_share)-1]
    candidate_names <- c(clean_fp_df[clean_fp_df$fp_vote_share == largest, "candidate_name"],
                         clean_fp_df[clean_fp_df$fp_vote_share == second_largest, "candidate_name"])
    clean_tcp_df <- data.frame(candidate_name = candidate_names,
                               other = NA,
                               division_name = "AUSTRALIAN CAPITAL TERRITORY",
                               state = "ACT",
                               year = elect_year,
                               last_name = toupper(str_extract(candidate_names, "[A-Za-z]+$")),
                               tcp_vote_share = c(max(clean_fp_df$fp_vote_share, na.rm = T),
                                                  100 - max(clean_fp_df$fp_vote_share, na.rm = T)))
  } else {
...
#1951, 1954, 1955, 1958, 1961, 1963, 1966, 1969, 1972, 1974, 1975, 1977, 1980, 1983, 1984, 1987...
