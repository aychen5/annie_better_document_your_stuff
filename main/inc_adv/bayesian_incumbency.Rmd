...
tpp_data <- tpp_data %>% 
  group_by(division) %>% 
  mutate(# number of "trials" per division
         total_years = n(), 
         # rescale incumbent predictor
         incumbent = if_else(incumbent == 1, 1, -1))
```


```{r}
get_prior(alp_win_t1 ~ 1 + incumbent + (1 + incumbent||division),  
          data = tpp_data, 
...
model2 <- brm(
  ...
  # total_years is number of years in each division (total number of trials)
    alp_win_t1 | trials(total_years) ~ 1 + incumbent + (1 + incumbent|division), 
...
tpp_data %>%
  ...
  # sample 100 fit lines from the posterior and overplot them
    ...
    facet_wrap(~division, nrow = 75) + # there are 150 divisions...way too many to plot...
